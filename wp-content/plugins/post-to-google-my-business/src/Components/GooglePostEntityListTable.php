...
class GooglePostEntityListTable extends PrefixedListTable {
	...
	function get_sortable_columns() {
		return [
//			'pgmb_location'     => ['pgmb_location', false],
//			'pgmb_status'       => ['pgmb_status', false],
		];
	}
	...
	function column_cb($item){
		return sprintf(
			'<input type="checkbox" name="pgmb_entity" value="%s" />', $item->get_id()
		);
	}
...
//		}
	...
	public function column_pgmb_location(GooglePostEntity $entity){
        try{
	        $this->api->set_user_id($entity->get_user_key());
	        $location = $this->api->get_location( NormalizeLocationName::from_with_account($entity->get_location_id())->without_account_id(), 'title' );
	        return $location->title;
        }catch(\Exception $e){
            return sprintf(__('Could not get location name: %s', 'post-to-google-my-business'), $e->getMessage());
        }
    }

    public function column_pgmb_status(GooglePostEntity $entity){
	    $state = $entity->get_post_state() ?: $entity->get_post_error() ?: __('Unknown', 'post-to-google-my-business');

	    $link = $entity->get_searchUrl() ? sprintf("- <a href='%s' target='_blank'>%s <span class=\"dashicons dashicons-external\"></span></a>", $entity->get_searchUrl(), __('View on Google', 'post-to-google-my-business')) : '';
...
//		echo '<input type="hidden" id="orderby" name="orderby" value="' . $this->_pagination_args['orderby'] . '" />';
		...
		?>
		<table class="wp-list-table pgmb-post-entities <?php echo implode( ' ', $this->get_table_classes() ); ?>">
			<thead>
			<tr>
